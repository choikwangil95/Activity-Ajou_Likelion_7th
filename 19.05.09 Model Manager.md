...
```
해당 코드는 django의 manager클래스에 대한 내용이다<br/>
해당 코드를 살펴보면 manager는 모델의 속성을 모두 가져오는 것을 objects로 표현해주는 것 같음
...
class Member(models.Model):
    ...
    objects = models.Manager()                      # objects를 지정해줘야한다.
    #image =
...
```python
class AuthorManager(models.Manager):                            # custom manager 추가하기
    def get_queryset(self):
        return super().get_queryset().filter(role='A')

class EditorManager(models.Manager):                            # custom manager 추가하기
    def get_queryset(self):
...
class Person(models.Model):
    ...
    people = models.Manager()                                   # manager 객체 만들어주기
    authors = AuthorManager()                                   # manager 객체 만들어주기
    editors = EditorManager()                                   # manager 객체 만들어주기
```
